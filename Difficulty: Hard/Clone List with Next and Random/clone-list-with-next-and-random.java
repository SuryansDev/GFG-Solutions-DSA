...
class Solution {
    HashMap<Node, Node> processed;
    Node nodeCloner(Node head){
        if(head == null) return null;
        if(!processed.containsKey(head)){
            Node clone = new Node(head.data);
            processed.put(head, clone);
            clone.next = nodeCloner(head.next);
            clone.random = nodeCloner(head.random);
        }
        return processed.get(head);
    }
    Node cloneLinkedList(Node head) {
        processed = new HashMap<>();
        return nodeCloner(head);
    }
}
